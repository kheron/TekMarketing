import type { ActivityLog, BrandContext, ContentFormat, ContentItem, PrismaClient } from '@prisma/client'
import { type ContentProposal, PlanningCycleResult, PlanningCycleResultSchema } from './types'
  ...
  for (const proposal of result.proposals) {
    const item = await db.contentItem.create({
      data: {
        ...
        format: toContentFormat(proposal.format),
...
function toContentFormat(format: ContentProposal['format']): ContentFormat {
  switch (format) {
    case 'POST':
    case 'THREAD':
      return format
...
function buildContextSummary(brand: BrandContext, activity: ActivityLog[], content: ContentItem[]) {
...
Do NOT use any platform or format values other than the ones explicitly allowed.`
  ...
  if (Array.isArray(rawProposals)) {
    parsed.proposals = rawProposals.map((proposal) => {
      const p = proposal as Record<string, unknown>
      return {
      platform: ['X', 'LINKEDIN'].includes(String(p.platform)) ? p.platform : 'X',
      format: ['POST', 'THREAD', 'CAROUSEL', 'IMAGE'].includes(String(p.format)) ? p.format : 'POST',
  ...
  if (!validated.success) {
    ...
    return {
      strategyNote: String(parsed.strategyNote || 'Generated content based on brand context.'),
      proposals: Array.isArray(parsed.proposals) && parsed.proposals.length ? parsed.proposals : [
        {
          platform: 'X',
          format: 'POST',
